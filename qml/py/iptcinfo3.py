...
class IPTCInfo:
    ...
    def save_as(self, newfile, options=None):
        ...
        if hasattr(tmpfh, 'getvalue'):  # StringIO
            ...
            tmpfh.close()
            oldExists = "false"
            if os.path.exists(newfile): # backup original file
                oldExists = "true"
                oldFileModificationDate = os.path.getmtime(newfile)
                #logger.error(oldFileModificationDate)
                extension = (newfile.split("."))[-1]
                dirName = os.path.dirname(newfile)
                shutil.move(newfile, dirName + '/' + 'backup_img.' + extension + '~')
            shutil.move(tmpfn, newfile)
            if "true" in oldExists:
                os.utime(newfile, (oldFileModificationDate, oldFileModificationDate))
            #newFileDate = os.path.getctime(newfile)
            #logger.error(newFileDate)
